...
func TestEmptyGenesis(t *testing.T) {
	state1 := v2customtypes.GenesisState{}
	require.True(t, state1.Empty())

	state2 := v2customtypes.DefaultGenesisState()
...
func TestValidateGenesis(t *testing.T) {
	...
	testCases := []struct {
		name         string
		genesisState func() *v2customtypes.GenesisState
		expErrMsg    string
	}{
		{
			name: "valid",
			genesisState: func() *v2customtypes.GenesisState {
				return v2customtypes.NewGenesisState(v1.DefaultStartingProposalID, params)
			},
		},
		{
			name: "invalid StartingProposalId",
			genesisState: func() *v2customtypes.GenesisState {
				return v2customtypes.NewGenesisState(0, params)
			},
		...
		},
		{
			...
			genesisState: func() *v2customtypes.GenesisState {
				...
				return v2customtypes.NewGenesisState(v1.DefaultStartingProposalID, params1)
			},
		...
		},
		{
			...
			genesisState: func() *v2customtypes.GenesisState {
				state := v2customtypes.NewGenesisState(v1.DefaultStartingProposalID, params)
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := v2customtypes.ValidateGenesis(tc.genesisState())
			if tc.expErrMsg != "" {
				require.Error(t, err)
