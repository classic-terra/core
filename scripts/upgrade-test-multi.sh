...
# CW20 demo requires newer CosmWasm. Only deploy at or after this core version.
CW20_MIN_VERSION=${CW20_MIN_VERSION:-"v2.3.3"}

# Determine threshold index in OLD_VERSIONS for CW20 deployment
CW20_MIN_IDX=-1
for idx in "${!OLD_VERSIONS[@]}"; do
    if [ "${OLD_VERSIONS[$idx]}" = "$CW20_MIN_VERSION" ]; then
        CW20_MIN_IDX=$idx
        break
    fi
done
if [ "$CW20_MIN_IDX" -lt 0 ]; then
    # If not found, default to a very large index to avoid premature deployment
    CW20_MIN_IDX=9999
fi
...
upload_and_instantiate_contract() {
    ...
    wait_for_tx "$binary_path" "$TXH"
    TXQ=$(${binary_path} q tx "$TXH" -o json 2>/dev/null || echo '{}')
    CW_CODE_ID=$(echo "$TXQ" | jq -r '.logs[0].events[] | select(.type=="store_code") | .attributes[] | select(.key=="code_id") | .value')
    echo "Contract uploaded with code ID: $CW_CODE_ID"
    ...
    # Build instantiate command and add --label only if supported
    CMD=("${binary_path}" tx wasm instantiate "$CW_CODE_ID" "$INIT_MSG" \
        --from test1 \
        --chain-id ${CHAIN_ID} \
        --gas auto \
        --gas-adjustment 1.3 \
        ...
        --output json)
    if ${binary_path} tx wasm instantiate --help 2>&1 | grep -q -- "--label"; then
        CMD+=(--label "Test CW20 Token")
    fi
    # Instantiate the contract
    TXH=$("${CMD[@]}" 2>>/tmp/tx_exec_error | jq -r '.txhash // empty')
    if [ -z "$TXH" ] || [ "$TXH" = "null" ]; then
        echo "CW20 instantiate failed; aborting." >&2
        return 1
    fi
    wait_for_tx "$binary_path" "$TXH"
    echo "CW20 instantiate txhash: ${TXH}" >&2
    # Extract contract address robustly
    CONTRACT_ADDR=""
    ATTEMPTS=0
    while [ $ATTEMPTS -lt 8 ] && { [ -z "$CONTRACT_ADDR" ] || [ "$CONTRACT_ADDR" = "null" ]; }; do
        # Try direct logs
        CAND=$(echo "$INIT_OUTPUT" | jq -r '[.logs[]? | .events[]? | .attributes[]? | select(.key=="_contract_address" or .key=="contract_address") | .value] | last // ""')
        ...
        # Try list-contract-by-code newest
        LIST_JSON=$(${binary_path} q wasm list-contract-by-code "$CW_CODE_ID" -o json 2>/dev/null || echo '{}')
        CAND=$(echo "$LIST_JSON" | jq -r '.contracts[-1] // ""')
        if [ -n "$CAND" ] && [ "$CAND" != "null" ]; then
            CONTRACT_ADDR="$CAND"
            break
        fi
    ...
    # Sanitize
    CONTRACT_ADDR=$(echo -n "$CONTRACT_ADDR" | tr -d '\n\r\t ')
...
execute_cw20_transfer() {
    ...
    TX_HASH=$(${binary_path} tx wasm execute $CONTRACT_ADDR "$TRANSFER_MSG" \
        --from test1 \
        --chain-id ${CHAIN_ID} \
        --gas auto \
        --gas-adjustment 1.3 \
    ...
    if [ -z "$TX_HASH" ] || [ "$TX_HASH" = "null" ]; then
        echo "CW20 transfer failed; aborting." >&2
        return 1
    fi
    wait_for_tx "$binary_path" "$TX_HASH"
    ${binary_path} q tx "$TX_HASH" -o json 2>/dev/null > /tmp/execution_logs
    ...
    BALANCE_MSG='{"balance":{"address":"'$TEST2_ADDR'"}}'
    BALANCE_QUERY=$(query_smart "${binary_path}" "$CONTRACT_ADDR" "$BALANCE_MSG")
    BALANCE=$(echo $BALANCE_QUERY | jq -r '.query_result.balance // .data.balance // "0"')
...
if [[ "$FORK" == "true" ]]; then
    ...
    for ((i=0; i<${#OLD_VERSIONS[@]}; i++)); do
        ...
        if [ $i -eq $((${#OLD_VERSIONS[@]}-1)) ]; then
            ...
            if [ $i -eq 0 ]; then
                if [ -f ${HOME}/cw20_contract_address.txt ]; then
                    ...
                    echo "Skipping CW20 transfer: CW20 demo not deployed yet (requires >= ${CW20_MIN_VERSION})"
                fi
            fi

            deploy_and_test_all_pools "_build/$NEXT_BINARY/terrad" "uluna" "uusd"

            # If CW20 demo not yet deployed and threshold version reached, deploy it now using NEXT binary
            if [ ! -f ${HOME}/cw20_contract_address.txt ]; then
                BIN_IDX=$(get_binary_index "$NEXT_BINARY")
                if [ "$BIN_IDX" -ge "$CW20_MIN_IDX" ]; then
                    echo "Deploying CW20 demo (meets min version index $CW20_MIN_IDX, current $BIN_IDX)"
                    upload_and_instantiate_contract "_build/$NEXT_BINARY/terrad" "${CW20_TOKEN_WASM}"
                else
                    echo "Skipping CW20 demo (current index $BIN_IDX < min $CW20_MIN_IDX)"
                fi
            fi
        fi
    done
...
# Also deploy and test Astroport pools after final upgrade
echo "Deploying and testing Astroport pools after final upgrade..."
