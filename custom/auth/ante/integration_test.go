		...
		s.Run(c.name, func() {
			...
			for i := 0; i < 1; i++ {
				...
				if i == 0 && c.expectedFeeAmount != 0 {
					s.Require().EqualError(err, fmt.Sprintf("0usdr is smaller than %dusdr: insufficient funds: insufficient funds", burnSplitRate.MulInt64(c.expectedFeeAmount).TruncateInt64()))
				} else {
					s.Require().NoError(err)
				}
