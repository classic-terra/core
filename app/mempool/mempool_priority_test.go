...
func (s *MempoolTestSuite) TestRandomGeneratedTxs() {
	s.iterations = 0
	s.mempool = mempool.NewPriorityMempool(mempool.PriorityOracleWithOnRead(func(tx sdk.Tx) {
		s.iterations++
	}))
	t := s.T()
	ctx := sdk.NewContext(nil, cmtproto.Header{}, false, log.NewNopLogger())
...
func TestNextSenderTx_TxLimit(t *testing.T) {
	...
	// unlimited
	mp := mempool.NewPriorityMempool(mempool.PriorityOracleWithMaxTx(0))
	for i, tx := range txs {
		c := ctx.WithPriority(tx.priority)
		require.NoError(t, mp.Insert(c, tx))
		require.Equal(t, i+1, mp.CountTx())
	...
	// limit: 3
	mp = mempool.NewPriorityMempool(mempool.PriorityOracleWithMaxTx(3))
	for i, tx := range txs {
		c := ctx.WithPriority(tx.priority)
		err := mp.Insert(c, tx)
		if i < 3 {
	...
	// disabled
	mp = mempool.NewPriorityMempool(mempool.PriorityOracleWithMaxTx(-1))
	for _, tx := range txs {
		c := ctx.WithPriority(tx.priority)
		err := mp.Insert(c, tx)
		require.NoError(t, err)
...
func TestNextSenderTx_TxReplacement(t *testing.T) {
	...
	feeBump := 20
	mp = mempool.NewPriorityMempool(mempool.PriorityOracleWithTxReplacement(func(op, np int64, oTx, nTx sdk.Tx) bool {
...
func TestPriorityMempool_OracleTx(t *testing.T) {
	// Setup test context and accounts
	ctx := sdk.NewContext(nil, cmtproto.Header{}, false, log.NewNopLogger())
	accounts := simtypes.RandomAccounts(rand.New(rand.NewSource(0)), 2)
	sa, sb := accounts[0].Address, accounts[1].Address

	// Define test transaction types
	txs := map[string]testTx{
		"regular": {
			priority: 20,
			nonce:    1,
			address:  sa,
		...
		"mixed": {
			...
			address:  sa,
			msgs: []sdk.Msg{
				&oracleexported.MsgAggregateExchangeRateVote{},
				&banktypes.MsgSend{},
			},
			id: 3,
		},
	...
	tests := []struct {
		name      string
		inputTxs  []string // References to txs map
		wantOrder []string // Expected order of transaction types
	}{
		{
			name:      "oracle tx should be processed first",
			inputTxs:  []string{"regular", "oracle"},
			wantOrder: []string{"oracle", "regular"},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create mempool with oracle priority option
			mp := mempool.NewPriorityMempool()

			// Insert test transactions
			for _, txKey := range tt.inputTxs {
				tx := txs[txKey]
				c := ctx.WithPriority(tx.priority)
				err := mp.Insert(c, tx)
				require.NoError(t, err, "failed to insert transaction %s", txKey)
			}

			// Verify count matches expected number of transactions
			...
			// Verify transaction order matches expected order
			require.Equal(t, tt.wantOrder, gotOrder,
				"transaction order doesn't match expected order")
		})
	}
}
