#!/bin/sh
# ADDITIONAL_SCRIPTS=scripts/wasm-deploy.sh bash scripts/upgrade-test.sh
...
for j in $(seq 0 1); do
	...
	out=$($BINARY tx wasm instantiate $id "$msg" --from test$j --output json --gas auto --no-admin --gas-adjustment 2.3 --fees 20000000uluna --chain-id $CHAIN_ID --home $HOME --keyring-backend $KEYRING_BACKEND -y)
	code=$(echo $out | jq -r '.code')
	if [ "$code" != "0" ]; then
