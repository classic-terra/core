# 2) Cryptographic Signatures Explained Simply (Public Bridge)

## What a Signature Is About
A cryptographic signature is digital proof that a message actually comes from the person or system named as sender, and that the content was not modified unnoticed in transit. In practical terms, anyone verifying a signed message can check whether sender and content match.
...
Without signatures, any party could claim that a message came from someone else. With signatures, it can be verified whether a message is authorized. This protects not only individual transactions but also the shared reliability of the network.
...
The key point is that this derivation is designed not to be practically reversible. The private key should not be derivable from the public key. At the same time, the relation between both keys remains strong enough for signature verification.
...
Bob verifies signature, message, and Alice's public key together. If verification succeeds, Bob can assume the message came from Alice and was not altered since signing. This is the normal case: signature verification creates binding between sender, content, and recipient.
