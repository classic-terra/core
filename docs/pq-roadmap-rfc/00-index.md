# Post-Quantum Roadmap RFC for Terra Classic

This directory contains the RFC text under development as chapter files.

## Chapters
- [01-executive-summary.md](./01-executive-summary.md) - Informative
- [02-cryptographic-signatures-explained.md](./02-cryptographic-signatures-explained.md) - Informative
- [03-current-state-and-terminology.md](./03-current-state-and-terminology.md) - Informative
- [04-target-states-and-options.md](./04-target-states-and-options.md) - Formal
- [05-roadmap-phases.md](./05-roadmap-phases.md) - Formal
- [06-audit-gates.md](./06-audit-gates.md) - Formal
- [07-live-chain-migration-paths.md](./07-live-chain-migration-paths.md) - Formal
- [08-governance-communication-rfc-process.md](./08-governance-communication-rfc-process.md) - Formal
- [09-open-decisions-decision-log.md](./09-open-decisions-decision-log.md) - Formal

## Scope Note
This document bundle describes direction, decision framework, and validation paths.
It is not an implementation or delivery plan.

## Reading Categories
The RFC uses two chapter categories:
- `Informative`: primarily written for non-technical readers and broader orientation.
- `Formal`: primarily written for technical and governance decision audiences.

This categorization is a reading aid, not an access barrier. Non-technical readers are explicitly encouraged to read formal chapters as well.
